...
void USART1_Init(void){	
	...
	*/
	//USART1 -> BRR |= 0x16C;	// 115200 APB2 freq = 42 MHz
        ...
        USART1 -> CR1 |= USART_CR1_RXNEIE | USART_CR1_TE | USART_CR1_RE;    // Прерывание по приему, вкл передачу и прием	
	...
	USART1 -> CR1 |= USART_CR1_UE;                                   // Включение USART1
        ...
        NVIC_EnableIRQ(USART1_IRQn);                                      // Разрешение ИМЕННОГО прерывания в контроллере NVIC
}
