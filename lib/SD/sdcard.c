...
SD_Error SD_Init(void)
{
  ...
  /* HCLK = 84 MHz, SDIOCLK = 84 MHz, SDIO_CK = HCLK/(2 + 2) = 21 MHz */  
...
SD_Error SD_PowerON(void)
{
  ...
  /* Power ON Sequence -------------------------------------------------------*/
  /* Configure the SDIO peripheral */
  SDIO_InitStructure.SDIO_ClockDiv = SDIO_INIT_CLK_DIV; /* HCLK = 84MHz, SDIOCLK = 84MHz, SDIO_CK = HCLK/(208 + 2) = 400 KHz */
  SDIO_InitStructure.SDIO_ClockEdge = SDIO_ClockEdge_Rising ;
  SDIO_InitStructure.SDIO_ClockBypass = SDIO_ClockBypass_Disable;
  SDIO_InitStructure.SDIO_ClockPowerSave = SDIO_ClockPowerSave_Disable;
  SDIO_InitStructure.SDIO_BusWide = SDIO_BusWide_1b;
...
SD_Error SD_ReadBlockBytes(uint32_t addr, uint8_t *readbuff_bytes, uint16_t BlockSizeBytes){
	...
	uint32_t *readbuff = (uint32_t *)(void *)readbuff_bytes;
	SD_Error errorstatus = SD_OK;
...
SD_Error SD_WriteBlockBytes(uint32_t addr, const uint8_t *writebuff_bytes, uint16_t BlockSizeBytes){
	SD_Error errorstatus = SD_OK;
	uint32_t *writebuff = (uint32_t *)(void *)writebuff_bytes;
...
/*static void DMA_TxConfiguration(uint32_t *BufferSRC, uint32_t BufferSize)
{
  ...
  // DMA2_Stream6 Channel4 disable 
  DMA_Cmd(DMA2_Channel4, DISABLE);

  // DMA2_Stream6 Channel4 Config 
